...
- **MiniMax H3 sequence execution (08-22):** segment guides can be combined into an ordered plan and executed with mixed FL2VA/REF2VA routing, source passthrough, disk resume, CPU-offloaded AV continuity, and concatenated image/audio outputs.
- **Director integrated executor and safe coexistence (08-20):** the optional executor provides native conditioning, sampling, and decode from a Director guide; the Director now uses the collision-safe `DaSiWaMiniMaxH3Director` node ID with migration for older DaSiWa workflows.
...
- **08-22:** sequence execution added through Segment, Plan, and Executor nodes, including mixed FL2VA/REF2VA routing, source passthrough, disk resume, CPU-offloaded continuity, and concatenated AV outputs.
- **08-20:** optional integrated Director Executor added; the Director moved to the collision-safe `DaSiWaMiniMaxH3Director` node ID, with schema-based migration for older DaSiWa workflows saved under `MiniMaxH3Director`.
